      ...
      if (part.inlineData && part.inlineData.data) {
        ...
        if (isOutfit) {
          if (imageData === base64Image) {
            ...
            // Auto-retry with Pro model if using Flash and user has enough XP
            if (preferredModel === 'gemini-2.5-flash-image') {
              console.log('⚠️ Attempting auto-retry with Nano Banana Pro (3 XP additional)');
              const retryConnection = await pool.getConnection();
              try {
                const [retryUsers] = await retryConnection.query<RowDataPacket[]>(
                  'SELECT xpoints FROM users WHERE id = ?',
                  [userId]
                );
                
                if (retryUsers[0].xpoints >= 3) {
                  // Deduct 3 more XP for Pro retry
                  await retryConnection.query(
                    'UPDATE users SET xpoints = xpoints - 3 WHERE id = ?',
                    [userId]
                  );
                  ...
                  if (retryPart?.inlineData?.data && retryPart.inlineData.data !== base64Image && retryPart.inlineData.data !== templateBase64) {
                    console.log('✅ Pro model retry succeeded!');
                    const retryBuffer = Buffer.from(retryPart.inlineData.data, 'base64');
                    const retryFilename = `generated-${Date.now()}.png`;
                    const retryFilepath = path.join(publicDir, retryFilename);
                    fs.writeFileSync(retryFilepath, retryBuffer);
                    
                    const retryImageUrl = `/api/generated/${retryFilename}`;
                    ...
                    try {
                      ...
                      const [insertResult]: any = await saveConnection.query(
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [userId, templateId, templateTitle, originalImageUrl, retryImageUrl, 6, isOutfit, aiPrompt, 'Nano Banana Pro (Auto-Retry)']
                      );
                      console.log('Generation saved with ID:', insertResult.insertId);
                    } finally {
                    ...
                    return NextResponse.json({
                      ...
                      modelUsed: 'Nano Banana Pro (Auto-Retry)',
                      generationId: null,
                      message: 'Generated successfully with Pro model after retry',
                    });
                  }
                }
              } finally {
            ...
            return NextResponse.json(
              { error: 'AI model failed to generate a new image. Please try again with Nano Banana Pro for better results.' },
              { status: 500 }
            );
          }
          if (templateBase64 && imageData === templateBase64) {
            ...
            // Auto-retry with Pro model if using Flash and user has enough XP
            if (preferredModel === 'gemini-2.5-flash-image') {
              console.log('⚠️ Template returned unchanged - attempting Pro model retry');
              const retryConnection = await pool.getConnection();
              try {
                const [retryUsers] = await retryConnection.query<RowDataPacket[]>(
                  'SELECT xpoints FROM users WHERE id = ?',
                ...
                if (retryUsers[0].xpoints >= 3) {
                  ...
                  console.log('🔄 Retrying with Nano Banana Pro');
                  const retryResponse = await ai.models.generateContent({
                    model: 'gemini-3-pro-image-preview',
                    contents: createUserContent(contentParts),
                  });
                  ...
                  const retryPart = retryResponse.candidates?.[0]?.content?.parts?.[0];
                  if (retryPart?.inlineData?.data && retryPart.inlineData.data !== base64Image && retryPart.inlineData.data !== templateBase64) {
                    console.log('✅ Pro retry succeeded!');
                    const retryBuffer = Buffer.from(retryPart.inlineData.data, 'base64');
                    const retryFilename = `generated-${Date.now()}.png`;
                    const retryFilepath = path.join(publicDir, retryFilename);
                    fs.writeFileSync(retryFilepath, retryBuffer);
            ...
            return NextResponse.json(
              { error: 'AI model failed to generate. It returned the template unchanged. Try Nano Banana Pro for better results.' },
              { status: 500 }
            );
          }
        }
